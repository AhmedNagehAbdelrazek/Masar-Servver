...
async function runInserts() {
  ...
  // ===== SUBSCRIPTION PLANS =====
  const planFree = await M.SubscriptionPlan.create({ name: 'Free Tier', periodDays: 30, percentageCut: 15, cost: 0, status: null, features: ['Post trips', '5 free completed trips/month'], isFree: true, freeOffer: { type: 'trips', max: 5 }, isActive: true });
  ...
  const s3 = await M.DriverSubscription.create({ driverId: d2.id, planId: planFree.id, planName: 'Free Tier', planPeriodDays: 30, planPercentageCut: 15, planCost: 0, balance: 0, paymentMethod: { name: 'None', type: 'bank_account', account_number: '-' }, freeOffer: { type: 'trips', max: 5 }, freeTripsUsed: 2, status: 'active', approvedAt: at(now - 15 * DAY), activatedAt: at(now - 15 * DAY), expiresAt: at(now + 15 * DAY) });
  ...
  const s6 = await M.DriverSubscription.create({ driverId: d5.id, planId: planAnnual.id, planName: 'Pro Annual', planPeriodDays: 365, planPercentageCut: 5, planCost: 150, balance: 0, screenshotId: imgIds[8], paymentMethod: bankSnap, status: 'active', approvedAt: at(now - 360 * DAY), activatedAt: at(now - 360 * DAY), expiresAt: at(now + 5 * DAY) });
  ...
  const b5 = await M.Booking.create({ tripId: t4.id, passengerId: p3.id, seatNumber: 3, seatsBooked: 1, agreedFare: 6, currency: 'JOD', dropoffOrder: 2, status: 'confirmed', referenceCode: 'MS000005E', paymentStatus: 'pending' });
  ...
  const b7 = await M.Booking.create({ tripId: t5.id, passengerId: p5.id, seatNumber: 2, seatsBooked: 1, agreedFare: 4, currency: 'JOD', status: 'cancelled', referenceCode: 'MS000007G', cancellationReason: 'Change of plans', cancelledBy: p5.id, cancelledAt: at(now - 6 * HOUR), paymentStatus: 'pending' });
