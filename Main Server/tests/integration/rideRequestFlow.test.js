...
async function createRideRequest(token, overrides = {}) {
  ...
  if (res.status !== 201) console.log('REQ CREATE DEBUG:', JSON.stringify(res.body));
  return res;
}
...
beforeEach(async () => {
  ...
  if (res.status !== 201) console.log('TRIP CREATE DEBUG:', JSON.stringify(res.body));
...
describe('POST /api/trips/:trip_id/offers/:offer_id/attach - deferred materialization', () => {
  ...
  it('should flip the trip to full when capacity reaches zero', async () => {
    ...
    if (single.status !== 201) console.log('TRIP CREATE DEBUG:', JSON.stringify(single.body));
