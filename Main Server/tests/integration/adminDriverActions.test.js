...
const {
  User, Vehicle, DriverProfile, UploadedImage, DocumentReview, Penalty,
...
describe('POST /api/admin/dashboard/drivers/:driver_id/status â€” direct writes', () => {
  ...
  it.each([
    ...
    const driver = await User.findByPk(DRIVER_ID);
    if (requested === 'blocked') expect(driver.status).toBe('banned');
    else expect(driver.status).toBe(requested);
  });
