...
describe('End-to-end user journey (driver + passenger full flow)', () => {
  it('runs the complete driver -> trip -> passenger -> ride -> rating journey', async () => {
    ...
    const driverSidePassengers = await getAgent()
      .get(`/api/trips/${tripId}/passengers`)
      .set('Authorization', `Bearer ${driverToken}`);
    const ratedPassenger = driverSidePassengers.body.passengers.find(
      (p) => p.passenger.full_name === 'Lina Passenger'
    );
    ...
    const completedInRecents = driverHomeAfter.body.recent_bookings.some(
      (b) => b.booking_id === bookingId && b.status === BOOKING_STATUS.COMPLETED
    );
