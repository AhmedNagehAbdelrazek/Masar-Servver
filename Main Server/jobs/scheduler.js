...
function findMissedSlots(task, { fromMs, toMs, toleranceMs }) {
  ...
  for (const t of scanMissedSlots(task, fromMs, toMs)) {
    if (toMs - t >= toleranceMs) missed.push(new Date(t));
  }
  ...
  for (const [name, { schedule, task }] of Object.entries(jobRegistry)) {
    ...
    cronTask.on('execution:missed', (context) => {
      ...
      cronTask.execute().catch((err) =>
        console.error(`[jobs] ${name} missed-run retry failed:`, err.message)
      );
    });

    if (catchUp) {
      runCatchUp(cronTask, name).catch((err) =>
        console.error(`[jobs] ${name} startup catch-up failed:`, err.message)
      );
    }
