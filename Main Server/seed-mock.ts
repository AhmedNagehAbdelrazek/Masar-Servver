...
const SALT_ROUNDS: number = parseInt(process.env.SALT_ROUNDS as string, 10) || 10;
...
async function resetTables(): Promise<void> {
  await (sequelize as unknown as { query: (sql: string) => Promise<void> }).query(`TRUNCATE TABLE ${TABLES.map((t) => `"${t}"`).join(', ')} RESTART IDENTITY CASCADE`);
}

const MARKER_PHONE: string = '+962790000001';

async function seedMockData(): Promise<boolean> {
  if (process.env.NODE_ENV === 'test') return false;
  if (process.env.SEED_MOCK_ON_BOOT !== 'true') return false;
  const marker = await (M as unknown as { User: { findOne: (o: unknown) => Promise<{ id: string } | null> } }).User.findOne({ where: { phone: MARKER_PHONE }, attributes: ['id'] });
  if (marker) {
...
async function runInserts(): Promise<void> {
  const passwordHash: string = await bcrypt.hash('Test@1234', SALT_ROUNDS);
  const adminPasswordHash: string = await bcrypt.hash(process.env.SEED_ADMIN_PASSWORD || 'Admin@1234', SALT_ROUNDS);

  const admin = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const moderator = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
    phone: '+962700000098',
  ...
  const d1 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const d2 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const d3 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const d4 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const d5 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const p1 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const p2 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const p3 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const p4 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  const p5 = await (M as unknown as { User: { create: (d: unknown) => Promise<{ id: string }> } }).User.create({
  ...
  console.log(`[mock-seed] users: ${await (M as unknown as { User: { count: () => Promise<number> } }).User.count()}`);

  const mkImage = async (n: number): Promise<string> =>
    (
      await (M as unknown as { UploadedImage: { create: (d: unknown) => Promise<{ id: string }> } }).UploadedImage.create({
  ...
  console.log(`[mock-seed] uploaded_images: ${await (M as unknown as { UploadedImage: { count: () => Promise<number> } }).UploadedImage.count()}`);

  await (M as unknown as { DriverProfile: { bulkCreate: (d: unknown[]) => Promise<void> } }).DriverProfile.bulkCreate([
  ...
  console.log(`[mock-seed] driver_profiles: ${await (M as unknown as { DriverProfile: { count: () => Promise<number> } }).DriverProfile.count()}`);

  await (M as unknown as { PassengerProfile: { bulkCreate: (d: unknown[]) => Promise<void> } }).PassengerProfile.bulkCreate([
  ...
  console.log(`[mock-seed] verification_status_changes: ${await (M as unknown as { VerificationStatusChange: { count: () => Promise<number> } }).VerificationStatusChange.count()}`);

  const planFree = await (M as unknown as { SubscriptionPlan: { create: (d: unknown) => Promise<{ id: string }> } }).SubscriptionPlan.create({ name: 'Free Tier', periodDays: 30, percentageCut: 15, cost: 0, status: null, features: ['Post trips', '5 free completed trips/month'], isFree: true, freeOffer: { type: 'trips', value: 5 }, isActive: true });
  const planWeekly = await (M as unknown as { SubscriptionPlan: { create: (d: unknown) => Promise<{ id: string }> } }).SubscriptionPlan.create({ name: 'Pro Weekly', periodDays: 7, percentageCut: 10, cost: 3, status: 'frequent', features: ['Lower commission', 'Priority support'], isFree: false, isActive: true });
  const planMonthly = await (M as unknown as { SubscriptionPlan: { create: (d: unknown) => Promise<{ id: string }> } }).SubscriptionPlan.create({ name: 'Pro Monthly', periodDays: 30, percentageCut: 8, cost: 16, status: 'popular', features: ['Lowest monthly commission', 'Featured trips', 'Priority support'], isFree: false, isActive: true });
  const planAnnual = await (M as unknown as { SubscriptionPlan: { create: (d: unknown) => Promise<{ id: string }> } }).SubscriptionPlan.create({ name: 'Pro Annual', periodDays: 365, percentageCut: 5, cost: 150, status: 'most_requested', features: ['Best commission rate', 'All Pro features', 'Badge'], isFree: false, isActive: true });
  await (M as unknown as { SubscriptionPlan: { create: (d: unknown) => Promise<void> } }).SubscriptionPlan.create({ name: 'Legacy Basic', periodDays: 30, percentageCut: 12, cost: 8, features: ['Legacy plan'], isFree: false, isActive: false });
  console.log(`[mock-seed] subscription_plans: ${await (M as unknown as { SubscriptionPlan: { count: () => Promise<number> } }).SubscriptionPlan.count()}`);

  await (M as unknown as { PaymentMethod: { bulkCreate: (d: unknown[]) => Promise<void> } }).PaymentMethod.bulkCreate([
  ...
  console.log(`[mock-seed] payment_methods: ${await (M as unknown as { PaymentMethod: { count: () => Promise<number> } }).PaymentMethod.count()}`);
  ...
  const s1 = await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<{ id: string }> } }).DriverSubscription.create({ driverId: d1.id, planId: planMonthly.id, planName: 'Pro Monthly', planPeriodDays: 30, planPercentageCut: 8, planCost: 16, balance: 0, screenshotId: imgIds[0], paymentMethod: cliqSnap, status: 'active', approvedAt: at(now - 10 * DAY), activatedAt: at(now - 10 * DAY), expiresAt: at(now + 20 * DAY) });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d1.id, planId: planMonthly.id, planName: 'Pro Monthly', planPeriodDays: 30, planPercentageCut: 8, planCost: 16, balance: 0, screenshotId: imgIds[1], paymentMethod: cliqSnap, status: 'expired', approvedAt: at(now - 40 * DAY), activatedAt: at(now - 40 * DAY), expiresAt: at(now - 10 * DAY) });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d2.id, planId: planFree.id, planName: 'Free Tier', planPeriodDays: 30, planPercentageCut: 15, planCost: 0, balance: 0, paymentMethod: { name: 'None', type: 'bank_account', account_number: '-' }, freeOffer: { type: 'trips', value: 5 }, freeTripsUsed: 2, status: 'active', approvedAt: at(now - 15 * DAY), activatedAt: at(now - 15 * DAY), expiresAt: at(now + 15 * DAY) });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d3.id, planId: planMonthly.id, planName: 'Pro Monthly', planPeriodDays: 30, planPercentageCut: 8, planCost: 16, balance: 16, screenshotId: imgIds[13], paymentMethod: cliqSnap, status: 'pending_approval', adminNotes: null });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d4.id, planId: planMonthly.id, planName: 'Pro Monthly', planPeriodDays: 30, planPercentageCut: 8, planCost: 16, balance: 0, screenshotId: imgIds[2], paymentMethod: bankSnap, status: 'expired', approvedAt: at(now - 60 * DAY), activatedAt: at(now - 60 * DAY), expiresAt: at(now - 30 * DAY) });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d5.id, planId: planAnnual.id, planName: 'Pro Annual', planPeriodDays: 365, planPercentageCut: 5, planCost: 150, balance: 0, screenshotId: imgIds[8], paymentMethod: bankSnap, status: 'active', approvedAt: at(now - 360 * DAY), activatedAt: at(now - 360 * DAY), expiresAt: at(now + 5 * DAY) });
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d5.id, planId: planWeekly.id, planName: 'Pro Weekly', planPeriodDays: 7, planPercentageCut: 10, planCost: 3, balance: 3, screenshotId: imgIds[9], paymentMethod: zainSnap, status: 'rejected', adminNotes: 'Screenshot unclear — transfer amount not visible.' });
  console.log(`[mock-seed] driver_subscriptions: ${await (M as unknown as { DriverSubscription: { count: () => Promise<number> } }).DriverSubscription.count()}`);

  await (M as unknown as { SubscriptionTransaction: { bulkCreate: (d: unknown[]) => Promise<void> } }).SubscriptionTransaction.bulkCreate([
  ...
  // addSeats(t5);
  // addSeats(t6, [2]);
  // await (M as unknown as { TripSeat: { bulkCreate: (d: unknown[]) => Promise<void> } }).TripSeat.bulkCreate(seatRows);
  // console.log(`[mock-seed] trip_seats: ${await (M as unknown as { TripSeat: { count: () => Promise<number> } }).TripSeat.count()}`);

  // await (M as unknown as { TripStop: { bulkCreate: (d: unknown[]) => Promise<void> } }).TripStop.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] trip_stops: ${await (M as unknown as { TripStop: { count: () => Promise<number> } }).TripStop.count()}`);

  // await (M as unknown as { TripAttribute: { bulkCreate: (d: unknown[]) => Promise<void> } }).TripAttribute.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] trip_attributes: ${await (M as unknown as { TripAttribute: { count: () => Promise<number> } }).TripAttribute.count()}`);

  // const b1 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t1.id, passengerId: p1.id, seatNumber: 2, seatsBooked: 1, agreedFare: 7, currency: 'JOD', dropoffPlace: 'University of Jordan', dropoffOrder: 1, status: 'completed', referenceCode: 'MS000001A', paymentStatus: 'paid_cash' });
  // const b2 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t1.id, passengerId: p2.id, seatNumber: 3, seatsBooked: 1, agreedFare: 7, currency: 'JOD', dropoffPlace: 'Irbid Downtown', dropoffOrder: 2, status: 'confirmed', referenceCode: 'MS000002B', paymentStatus: 'pending' });
  // const b3 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t2.id, passengerId: p2.id, seatNumber: 2, seatsBooked: 1, agreedFare: 18, currency: 'JOD', status: 'confirmed', referenceCode: 'MS000003C', paymentStatus: 'paid_cash' });
  // const b4 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t4.id, passengerId: p1.id, seatNumber: 2, seatsBooked: 1, agreedFare: 6, currency: 'JOD', dropoffPlace: 'Jerash Visitor Center', dropoffOrder: 1, status: 'confirmed', referenceCode: 'MS000004D', paymentStatus: 'pending' });
  // await (M as unknown as { Booking: { create: (d: unknown) => Promise<void> } }).Booking.create({ tripId: t4.id, passengerId: p3.id, seatNumber: 3, seatsBooked: 1, agreedFare: 6, currency: 'JOD', dropoffOrder: 2, status: 'confirmed', referenceCode: 'MS000005E', paymentStatus: 'pending' });
  // const b6 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t4.id, passengerId: p4.id, seatNumber: 4, seatsBooked: 1, agreedFare: 6, currency: 'JOD', dropoffOrder: 3, status: 'pending', referenceCode: 'MS000006F', paymentStatus: 'pending' });
  // await (M as unknown as { Booking: { create: (d: unknown) => Promise<void> } }).Booking.create({ tripId: t5.id, passengerId: p5.id, seatNumber: 2, seatsBooked: 1, agreedFare: 4, currency: 'JOD', status: 'cancelled', referenceCode: 'MS000007G', cancellationReason: 'Change of plans', cancelledBy: p5.id, cancelledAt: at(now - 6 * HOUR), paymentStatus: 'pending' });
  // const b8 = await (M as unknown as { Booking: { create: (d: unknown) => Promise<{ id: string }> } }).Booking.create({ tripId: t6.id, passengerId: p3.id, seatNumber: 2, seatsBooked: 1, agreedFare: 4, currency: 'JOD', status: 'confirmed', referenceCode: 'MS000008H', paymentStatus: 'pending' });
  // console.log(`[mock-seed] bookings: ${await (M as unknown as { Booking: { count: () => Promise<number> } }).Booking.count()}`);

  // await (M as unknown as { Rating: { bulkCreate: (d: unknown[]) => Promise<void> } }).Rating.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] ratings: ${await (M as unknown as { Rating: { count: () => Promise<number> } }).Rating.count()}`);

  // await (M as unknown as { DelayEvent: { bulkCreate: (d: unknown[]) => Promise<void> } }).DelayEvent.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] delay_events: ${await (M as unknown as { DelayEvent: { count: () => Promise<number> } }).DelayEvent.count()}`);

  // const c1 = await (M as unknown as { Complaint: { create: (d: unknown) => Promise<{ id: string }> } }).Complaint.create({ reporterId: p5.id, accusedId: d5.id, category: 'cancellation_dispute', description: 'Driver cancelled two hours before departure with no explanation.', evidenceUrls: ['https://res.cloudinary.com/demo/image/uploads/evidence_1.jpg'], status: 'resolved', resolution: 'Reviewed chat logs; driver had a valid emergency. Warning issued.', resolvedBy: admin.id, resolvedAt: at(now - 4 * HOUR) });
  // const c2 = await (M as unknown as { Complaint: { create: (d: unknown) => Promise<{ id: string }> } }).Complaint.create({ bookingId: b6.id, reporterId: p3.id, accusedId: d4.id, category: 'misconduct', description: 'Smoked in the car despite the no-smoking attribute.', status: 'resolved', resolution: 'Confirmed by both passengers. Driver suspended for 7 days.', resolvedBy: admin.id, resolvedAt: at(now - 1 * DAY) });
  // const c3 = await (M as unknown as { Complaint: { create: (d: unknown) => Promise<{ id: string }> } }).Complaint.create({ reporterId: p2.id, accusedId: p4.id, category: 'noise', description: 'Passenger was loud and on phone calls the whole ride.', status: 'reviewing' });
  // const c4 = await (M as unknown as { Complaint: { create: (d: unknown) => Promise<{ id: string }> } }).Complaint.create({ reporterId: p1.id, accusedId: d1.id, category: 'route_deviation', description: 'Took a longer route without asking.', status: 'open' });
  // const c5 = await (M as unknown as { Complaint: { create: (d: unknown) => Promise<{ id: string }> } }).Complaint.create({ bookingId: b6.id, reporterId: d1.id, accusedId: p4.id, category: 'no_show', description: 'Passenger never showed up at pickup and did not cancel.', status: 'open' });
  // console.log(`[mock-seed] complaints: ${await (M as unknown as { Complaint: { count: () => Promise<number> } }).Complaint.count()}`);

  // await (M as unknown as { Penalty: { bulkCreate: (d: unknown[], o: unknown) => Promise<void> } }).Penalty.bulkCreate([
  ...
  // ], { returning: false });
  // console.log(`[mock-seed] penalties: ${await (M as unknown as { Penalty: { count: () => Promise<number> } }).Penalty.count()}`);

  // const tickets = await (M as unknown as { SupportTicket: { bulkCreate: (d: unknown[]) => Promise<Array<{ id: string }>> } }).SupportTicket.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] support_tickets: ${await (M as unknown as { SupportTicket: { count: () => Promise<number> } }).SupportTicket.count()}`);

  // await (M as unknown as { Message: { bulkCreate: (d: unknown[]) => Promise<void> } }).Message.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] messages: ${await (M as unknown as { Message: { count: () => Promise<number> } }).Message.count()}`);

  // await (M as unknown as { Notification: { bulkCreate: (d: unknown[]) => Promise<void> } }).Notification.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] notifications: ${await (M as unknown as { Notification: { count: () => Promise<number> } }).Notification.count()}`);

  // await (M as unknown as { NotificationSetting: { bulkCreate: (d: unknown[]) => Promise<void> } }).NotificationSetting.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] notification_settings: ${await (M as unknown as { NotificationSetting: { count: () => Promise<number> } }).NotificationSetting.count()}`);

  // await (M as unknown as { FavoriteDriver: { bulkCreate: (d: unknown[]) => Promise<void> } }).FavoriteDriver.bulkCreate([
  ...
  // ]);
  // await (M as unknown as { FavoriteRoute: { bulkCreate: (d: unknown[]) => Promise<void> } }).FavoriteRoute.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] favorites: drivers=${await (M as unknown as { FavoriteDriver: { count: () => Promise<number> } }).FavoriteDriver.count()} routes=${await (M as unknown as { FavoriteRoute: { count: () => Promise<number> } }).FavoriteRoute.count()}`);

  // const r1 = await (M as unknown as { RideRequest: { create: (d: unknown) => Promise<{ id: string }> } }).RideRequest.create({ passengerId: p1.id, originPlace: 'Ras El Ain', originCity: 'Amman', originLat: 31.9560, originLng: 35.9330, originTime: at(now + 22 * HOUR), destinationPlace: 'Sweifieh', destinationCity: 'Amman', destinationLat: 31.9520, destinationLng: 35.8620, seatsNeeded: 1, maxBudget: 5, currency: 'JOD', attributesPreferred: { ac: true }, status: 'open', expiresAt: at(now + 26 * HOUR) });
  // const r2 = await (M as unknown as { RideRequest: { create: (d: unknown) => Promise<{ id: string }> } }).RideRequest.create({ passengerId: p2.id, originPlace: '7th Circle', originCity: 'Amman', originTime: at(now + 2 * DAY), destinationPlace: 'Aqaba City Center', destinationCity: 'Aqaba', arrivalDeadline: at(now + 2 * DAY + 5 * HOUR), seatsNeeded: 2, maxBudget: 40, currency: 'JOD', status: 'offered', expiresAt: at(now + 2 * DAY) });
  // const r3 = await (M as unknown as { RideRequest: { create: (d: unknown) => Promise<{ id: string }> } }).RideRequest.create({ passengerId: p3.id, originPlace: 'Dabouq', originCity: 'Amman', originTime: at(now + 3 * DAY), destinationPlace: 'Jerash Visitor Center', destinationCity: 'Jerash', seatsNeeded: 1, maxBudget: 8, currency: 'JOD', status: 'accepted', expiresAt: at(now + 3 * DAY) });
  // await (M as unknown as { RideRequest: { create: (d: unknown) => Promise<void> } }).RideRequest.create({ passengerId: p4.id, originPlace: 'Zarqa Downtown', originCity: 'Zarqa', originTime: at(now - 2 * DAY), destinationPlace: 'Mahes', destinationCity: 'Amman', seatsNeeded: 1, maxBudget: 4, status: 'expired', expiresAt: at(now - 1 * DAY) });
  // const r5 = await (M as unknown as { RideRequest: { create: (d: unknown) => Promise<{ id: string }> } }).RideRequest.create({ passengerId: p5.id, originPlace: 'Wakalat St', originCity: 'Amman', originTime: at(now + 30 * HOUR), destinationPlace: 'Dead Sea Amman Beach', destinationCity: 'Dead Sea', seatsNeeded: 1, maxBudget: 7, status: 'cancelled', expiresAt: at(now + 34 * HOUR) });

  // await (M as unknown as { RequestOffer: { bulkCreate: (d: unknown[]) => Promise<void> } }).RequestOffer.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] ride_requests: ${await (M as unknown as { RideRequest: { count: () => Promise<number> } }).RideRequest.count()}, request_offers: ${await (M as unknown as { RequestOffer: { count: () => Promise<number> } }).RequestOffer.count()}`);

  // await (M as unknown as { AuditLog: { bulkCreate: (d: unknown[]) => Promise<void> } }).AuditLog.bulkCreate([
  ...
  // ]);
  // console.log(`[mock-seed] audit_logs: ${await (M as unknown as { AuditLog: { count: () => Promise<number> } }).AuditLog.count()}`);
}
  ...
  (async (): Promise<void> => {
    try {
      ...
      try {
        ...
        console.error('[mock-seed] could not truncate tables — run `npm run db:init` first.', msg);
        process.exit(1);
      }
      await runInserts();
      console.log('');
