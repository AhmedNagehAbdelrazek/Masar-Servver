...
async function runInserts(): Promise<void> {
  ...
  await (M as unknown as { DriverSubscription: { create: (d: unknown) => Promise<void> } }).DriverSubscription.create({ driverId: d5.id, planId: planWeekly.id, planName: 'Pro Weekly', planPeriodDays: 7, planPercentageCut: 10, cost: 3, balance: 3, screenshotId: imgIds[9], paymentMethod: zainSnap, status: 'rejected', adminNotes: 'Screenshot unclear — transfer amount not visible.' });
