...
async function aggregate(driverId, { period = 'month', from, to } = {}) {
  ...
  if (!fromDate) {
    ...
    if (truncated === 'day') fromDate.setHours(0, 0, 0, 0);
    else if (truncated === 'week') fromDate.setDate(now.getDate() - 7);
    else fromDate.setMonth(now.getMonth() - 1);
  }
  if (!toDate) toDate = new Date(now);

  const fmt = truncated === 'month' ? 'YYYY-MM' : 'YYYY-MM-DD';
  const bucketExpr = sequelize.fn(
    'to_char',
    sequelize.fn('date_trunc', truncated, sequelize.col('Booking.createdat')),
    fmt
  );
