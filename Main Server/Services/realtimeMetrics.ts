...
function recordConnection(userId) {
  ...
  if (userId) state.activeUsers.add(userId);
...
function getSnapshot(io) {
  ...
  const connectionFailureRate =
    windowAttempts.length > 0 ? windowFailures / windowAttempts.length : 0;

  const reconnectRate1m =
    (state.reconnects.samples.length +
...
export { recordConnection, recordDisconnection, recordConnectionFailure, recordReconnect, recordDelivery, recordFailure, recordRateLimited, recordEvent, getSnapshot, reset, WINDOW_MS, SLOW_DELIVERY_MS, ALERT_THRESHOLD };
