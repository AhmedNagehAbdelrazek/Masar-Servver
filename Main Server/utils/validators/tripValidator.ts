...
export const createTripValidation: ValidationChain[] = [
  ...
  body('repeated_days')
    ...
    .custom((days: unknown) => {
      if (!Array.isArray(days) || !(days as number[]).every((d) => Number.isInteger(d) && d >= 0 && d <= 6)) {
  ...
  body('fare_per_seat')
    ...
    .custom((value: unknown) => {
      if (parseFloat(String(value)) < 0) throw new Error(V.FARE_CANNOT_BE_NEGATIVE);
      return true;
    }),
  ...
  body('instructions')
    ...
    .custom((arr: unknown) => {
      if (!Array.isArray(arr) || !(arr as unknown[]).every((item) => typeof item === 'string' && item.trim().length > 0 && item.trim().length <= 1000)) {
        throw new Error(V.EACH_INSTRUCTION_MUST_BE_A_NON_EMPTY_STRING_OF_AT_MOST_1000);
      }
      return true;
    })
    .customSanitizer((arr: unknown) => (arr as string[]).map((s) => s.trim())),
...
export const searchAvailableTripsValidation: ValidationChain[] = [
  query('origin_city')
    .optional()
    .trim()
    .isLength({ max: 100 }).withMessage(V.ORIGIN_CITY_MUST_BE_AT_MOST_100_CHARACTERS),
  ...
  query('time_to')
    ...
    .matches(/^([01]\d|2[0-3]):([0-5]\d)$/).withMessage(V.DEPARTURE_TIME_MUST_BE_IN_HH_MM_FORMAT)
    .custom((value: unknown, { req }: Meta) => {
      if ((req as unknown as { query: Record<string, unknown> }).query.time_from && String(value) < String((req as unknown as { query: Record<string, unknown> }).query.time_from)) {
...
export const tripParamValidation: ValidationChain[] = [
  param('trip_id')
    .isUUID().withMessage(V.TRIP_ID_MUST_BE_A_VALID_UUID),
];

export const tripPassengersValidation: ValidationChain[] = [
  ...tripParamValidation,
  query('status')
    .optional()
...
export const lockSeatValidation: ValidationChain[] = [
  param('trip_id')
    .isUUID().withMessage(V.TRIP_ID_MUST_BE_A_VALID_UUID),
  body('seat_number')
...
const _exported = { createTripValidation, searchAvailableTripsValidation, lockSeatValidation, releaseSeatLockValidation, updateTripValidation, tripParamValidation, tripPassengersValidation, cancelTripValidation };
...
if (typeof module !== 'undefined' && module.exports) {
  // @ts-ignore
  module.exports = { createTripValidation, searchAvailableTripsValidation, lockSeatValidation, releaseSeatLockValidation, updateTripValidation, tripParamValidation, tripPassengersValidation, cancelTripValidation };
  // @ts-ignore
  module.exports.createTripValidation = createTripValidation;
  // @ts-ignore
  module.exports.searchAvailableTripsValidation = searchAvailableTripsValidation;
  // @ts-ignore
  module.exports.lockSeatValidation = lockSeatValidation;
  // @ts-ignore
  module.exports.releaseSeatLockValidation = releaseSeatLockValidation;
  // @ts-ignore
  module.exports.updateTripValidation = updateTripValidation;
  // @ts-ignore
  module.exports.tripParamValidation = tripParamValidation;
  // @ts-ignore
  module.exports.tripPassengersValidation = tripPassengersValidation;
  // @ts-ignore
  module.exports.cancelTripValidation = cancelTripValidation;
  // @ts-ignore
