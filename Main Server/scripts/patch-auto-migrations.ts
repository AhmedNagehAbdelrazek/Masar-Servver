...
if (!fs.existsSync(target)) {
  // Allow import without side-effects when module is required elsewhere
  // but keep process alive for script execution path.
} else {
  let content: string = fs.readFileSync(target, 'utf8');
  const hadCRLF: boolean = content.includes('\r\n');
  if (hadCRLF) {
  ...
  const rawReplacement =
    'let attributes = models[model].rawAttributes || models[model].attributes;';
