...
function getVersionsDir() {
    return path_1.default.join(__dirname, '..', 'versions');
}
...
async function executeCommand(queryInterface, command, index) {
    ...
    const fn = queryInterface[command.fn];
    if (typeof fn !== 'function') {
        throw new Error(`[migrate] Unknown migration command fn: ${command.fn}`);
    }
    return fn.apply(queryInterface, command.params);
}
...
async function runMigrationFileV2(migration, queryInterface, SequelizeLib) {
...
async function runMigrations(options = {}) {
    ...
    const sequelize = options.sequelize || (0, connection_1.createConnection)(options);
    ...
    try {
        ...
        for (const file of pending) {
            ...
            const skippedHere = await runMigrationFileV2(migration, queryInterface, SequelizeLib);
...
exports.default = { runMigrations, getVersionsDir };
