...
async function resetTables() {
    await database_1.default.query(`TRUNCATE TABLE ${TABLES.map((t) => `"${t}"`).join(', ')} RESTART IDENTITY CASCADE`);
}
const MARKER_PHONE = '+962790000001';
async function seedMockData() {
    if (process.env.NODE_ENV === 'test')
        return false;
    if (process.env.SEED_MOCK_ON_BOOT !== 'true')
        return false;
...
async function runInserts() {
    const passwordHash = await bcrypt_1.default.hash('Test@1234', SALT_ROUNDS);
    const adminPasswordHash = await bcrypt_1.default.hash(process.env.SEED_ADMIN_PASSWORD || 'Admin@1234', SALT_ROUNDS);
    ...
    const mkImage = async (n) => (await M.UploadedImage.create({
    ...
    await M.DriverSubscription.create({ driverId: d5.id, planId: planWeekly.id, planName: 'Pro Weekly', planPeriodDays: 7, planPercentageCut: 10, cost: 3, balance: 3, screenshotId: imgIds[9], paymentMethod: zainSnap, status: 'rejected', adminNotes: 'Screenshot unclear — transfer amount not visible.' });
