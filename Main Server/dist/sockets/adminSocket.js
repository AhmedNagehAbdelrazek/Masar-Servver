...
module.exports = (io, socket) => {
    ...
    socket.on('admin:ticket_assign', async (payload, ack) => {
        try {
            ...
            if (!payload || !payload.agent_id)
                throw ApiErrors.validation('AGENT_ID_IS_REQUIRED');
            ...
            if (ack)
                ack(ok({ support_ticket_id: ticket.id, assigned_to: payload.agent_id }));
        }
        catch (err) {
            if (ack)
    ...
    socket.on('admin:ticket_resolve', async (payload, ack) => {
        try {
            ...
            if (ack)
                ack(ok({ support_ticket_id: ticket.id, status: TICKET_STATUS.RESOLVED }));
        }
        catch (err) {
            if (ack)
                ack(errorFromApiError(err));
