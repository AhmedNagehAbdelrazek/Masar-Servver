...
function validateFreeOffer(value, { req }) {
    ...
    if (isFree) {
        if (value === undefined || value === null || typeof value !== 'object') {
            throw new Error(validation_keys_1.default.FREE_OFFER_IS_REQUIRED_FOR_A_FREE_PLAN);
        }
...
function validateFreeOfferShape(offer) {
    ...
    if (!Number.isFinite(v) || v <= 0) {
        throw new Error(validation_keys_1.default.FREE_OFFER_VALUE_MUST_BE_A_POSITIVE_NUMBER);
    }
    return true;
}
function validateEmail(value) {
    ...
    if (typeof value !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
        throw new Error(validation_keys_1.default.EMAIL_MUST_BE_VALID);
    }
    return true;
}
exports.createPlanValidation = [
    (0, express_validator_1.body)('name')
        .trim()
        .notEmpty().withMessage(validation_keys_1.default.PLAN_NAME_IS_REQUIRED)
        .isLength({ max: 100 }).withMessage(validation_keys_1.default.PLAN_NAME_MUST_BE_AT_MOST_100_CHARACTERS),
    (0, express_validator_1.body)('period_days')
        .isInt({ min: 1 }).withMessage(validation_keys_1.default.PERIOD_DAYS_MUST_BE_A_POSITIVE_INTEGER),
    (0, express_validator_1.body)('percentage_cut')
        .optional()
        .isDecimal({ decimal_digits: '1,2' }).withMessage(validation_keys_1.default.PERCENTAGE_CUT_MUST_BE_A_DECIMAL)
        .custom((value) => {
        const n = parseFloat(String(value));
        if (n < 0 || n > 100)
            throw new Error(validation_keys_1.default.PERCENTAGE_CUT_MUST_BE_BETWEEN_0_AND_100);
        return true;
    }),
    (0, express_validator_1.body)('cost')
        .optional()
        .isDecimal({ decimal_digits: '1,2' }).withMessage(validation_keys_1.default.COST_MUST_BE_A_DECIMAL)
        .custom((value) => {
        if (parseFloat(String(value)) < 0)
            throw new Error(validation_keys_1.default.COST_CANNOT_BE_NEGATIVE);
        return true;
    }),
    ...
    (0, express_validator_1.body)('features')
        .optional()
        .isArray().withMessage(validation_keys_1.default.FEATURES_MUST_BE_AN_ARRAY)
        .custom((arr) => {
        if (!Array.isArray(arr) || !arr.every((f) => typeof f === 'string' && f.trim().length <= 200)) {
            throw new Error(validation_keys_1.default.EACH_FEATURE_MUST_BE_A_STRING_OF_AT_MOST_200_CHARACTERS);
        }
        return true;
    })
    ...
    (0, express_validator_1.body)('is_free')
        .optional()
        .isBoolean().withMessage(validation_keys_1.default.IS_FREE_MUST_BE_A_BOOLEAN)
        .toBoolean(),
...
exports.planIdValidation = [
    (0, express_validator_1.param)('plan_id').isUUID().withMessage(validation_keys_1.default.PLAN_ID_MUST_BE_A_VALID_UUID),
];
exports.createPaymentMethodValidation = [
    (0, express_validator_1.body)('name')
        .trim()
        .notEmpty().withMessage(validation_keys_1.default.METHOD_NAME_IS_REQUIRED)
        .isLength({ max: 100 }).withMessage(validation_keys_1.default.METHOD_NAME_MUST_BE_AT_MOST_100_CHARACTERS),
    (0, express_validator_1.body)('account_number')
        .trim()
        .notEmpty().withMessage(validation_keys_1.default.ACCOUNT_NUMBER_IS_REQUIRED_2)
        .isLength({ max: 50 }).withMessage(validation_keys_1.default.ACCOUNT_NUMBER_MUST_BE_AT_MOST_50_CHARACTERS_2),
    (0, express_validator_1.body)('type')
        .isIn(Object.values(constants_1.PAYMENT_METHOD_TYPE)).withMessage(validation_keys_1.default.TYPE_MUST_BE_ONE_OF_BANK_ACCOUNT_E_WALLET_MOBILE_MONEY_2),
...
exports.paymentMethodIdValidation = [
    (0, express_validator_1.param)('method_id').isUUID().withMessage(validation_keys_1.default.PAYMENT_METHOD_ID_MUST_BE_A_VALID_UUID),
];
