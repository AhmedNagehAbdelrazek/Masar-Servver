...
const createTripValidation = [
    ...
    body('origin_lat')
        ...
        if (lat < -90 || lat > 90)
            throw new Error(V.ORIGIN_LATITUDE_MUST_BE_BETWEEN_90_AND_90);
        return true;
    }),
    body('origin_lng')
        .optional()
        ...
        if (lng < -180 || lng > 180)
            throw new Error(V.ORIGIN_LONGITUDE_MUST_BE_BETWEEN_180_AND_180);
        return true;
    }),
    ...
    body('destination_lat')
        ...
        const lat = parseFloat(value);
        if (lat < -90 || lat > 90)
            throw new Error(V.DESTINATION_LATITUDE_MUST_BE_BETWEEN_90_AND_90);
        return true;
    }),
    body('destination_lng')
        ...
        const lng = parseFloat(value);
        if (lng < -180 || lng > 180)
            throw new Error(V.DESTINATION_LONGITUDE_MUST_BE_BETWEEN_180_AND_180);
        return true;
    }),
    body('waypoints')
        .optional()
    ...
    body('departure_date')
        ...
        if (date < today)
            throw new Error(V.DEPARTURE_DATE_MUST_BE_TODAY_OR_IN_THE_FUTURE);
        return true;
    }),
    body('departure_time')
    ...
    body('repeated_end_date')
        ...
        if (endDate <= startDate)
            throw new Error(V.END_DATE_MUST_BE_AFTER_DEPARTURE_DATE);
        return true;
    }),
    body('allowed_type')
        .optional()
    ...
    body('fare_per_seat')
        ...
        if (parseFloat(value) < 0)
            throw new Error(V.FARE_CANNOT_BE_NEGATIVE);
        return true;
    }),
    body('seats')
...
const updateTripValidation = [
    ...
    body('departure_time')
        ...
        if (new Date(value) <= new Date())
            throw new Error(V.DEPARTURE_TIME_MUST_BE_IN_THE_FUTURE);
        return true;
    }),
    body('arrival_time')
        .optional()
