...
exports.MODES = ['en', 'ar', 'both'];
exports.DEFAULT_MODE = 'ar';
function normalizeMode(raw) {
    if (raw == null)
        return null;
    const value = String(raw).trim().toLowerCase();
    return exports.MODES.includes(value) ? value : null;
}
function defaultMode() {
    return normalizeMode(process.env.APP_LOCALE) || exports.DEFAULT_MODE;
}
...
function fill(text, params) {
    if (!params || !text.includes('{'))
        return text;
    return text.replace(/\{(\w+)\}/g, (match, name) => params[name] != null
        ? String(params[name])
        : match);
}
/**
...
function entryFor(key) {
    const catalog = messages_1.MESSAGES;
...
function t(key, params, mode = defaultMode()) {
    ...
    return fill(typedEntry[mode] != null ? typedEntry[mode] : typedEntry.en || String(key), params);
}
/**
...
function tValidation(text, mode = defaultMode()) {
    const catalog = messages_1.VALIDATION_MESSAGES;
