...
describe('expirySweepJob', () => {
    it('flips subscriptions, recomputes balances, unpublishes trips, and notifies', async () => {
        ...
        expect(notificationService.sendToUser).toHaveBeenCalledWith({ id: 'driver-1', locale: 'ar' }, 'PLAN_EXPIRED', expect.objectContaining({ channels: ['in_app', 'push'], vars: { plan: 'Basic' } }));
...
describe('expiryReminderJob', () => {
    it('notifies each driver once about their earliest expiring plan', async () => {
        ...
        expect(notificationService.sendToUser).toHaveBeenCalledWith({ id: 'driver-1', locale: 'en' }, 'PLAN_EXPIRING_SOON', expect.objectContaining({ channels: ['in_app', 'push'], vars: { plan: 'Basic' } }));
        expect(notificationService.sendToUser).toHaveBeenCalledWith({ id: 'driver-2', locale: 'en' }, 'PLAN_EXPIRING_SOON', expect.objectContaining({ vars: { plan: 'Pro' } }));
...
describe('lowBalanceWarningJob', () => {
    it('warns only drivers whose upcoming trip balance cannot cover commission', async () => {
        ...
        expect(notificationService.sendToUser).toHaveBeenCalledWith({ id: 'driver-1', locale: 'en' }, 'LOW_BALANCE_WARNING', expect.objectContaining({ channels: ['in_app', 'push'], vars: { route: 'Irbid' } }));
