...
describe('wrapTask', () => {
    it('records the last run after a successful execution', async () => {
        ...
        expect(redis.setKey).toHaveBeenCalledWith('jobs:lastrun:expirySweep', expect.any(String), 3 * 24 * 60 * 60);
    });
...
describe('getLastRun / recordLastRun', () => {
    ...
    it('records an ISO timestamp with a TTL', async () => {
        ...
        expect(redis.setKey).toHaveBeenCalledWith('jobs:lastrun:lowBalanceWarning', '2026-08-09T09:00:00.000Z', 3 * 24 * 60 * 60);
    });
});
describe('scheduleJobs', () => {
    ...
    it('schedules enabled jobs with overlap prevention and a timezone', () => {
        ...
        expect(cron.schedule).toHaveBeenCalledWith('0 9 * * *', expect.any(Function), expect.objectContaining({ name: 'reminder', noOverlap: true, timezone: expect.any(String) }));
