...
const activateSub = (subPlan, balance, opts = {}) => DriverSubscription.create({
    driverId: DRIVER_ID,
    planId: subPlan.id,
...
describe('US4 - Expiry transition', () => {
    it('sweeps expired plans: flips status, recomputes balance, unpublishes trips, notifies', async () => {
        ...
        await DriverSubscription.update({ expiresAt: new Date(Date.now() - 60 * 1000) }, { where: { id: sub.id } });
