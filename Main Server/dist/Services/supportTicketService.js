...
const { SupportTicket, SupportTicketMessage, User, Booking, } = require('../Models');
...
async function notifyStaff(ticket) {
    ...
    await Promise.allSettled(staff.map((member) => notificationService.sendToUser(member, 'SUPPORT_TICKET_NEW', {
        channels: ['in_app'],
        vars: {
            reference_code: ticket.referenceCode,
            subject: ticket.subject,
...
async function createTicket(user, payload) {
    ...
    if (payload.booking_id) {
        ...
        if (!booking)
            throw ApiErrors.notFound('REFERENCED_BOOKING_NOT_FOUND');
    }
...
async function getTicket(user, ticketId) {
    ...
    if (!ticket)
        throw ApiErrors.notFound('SUPPORT_TICKET_NOT_FOUND');
